...
def test_golden_gate_assembly():

    desired_sequence = ''
    sequences = {}
    for i in range(1, 4):
        sequence_path = os.path.join("data", f"test_sequence_spai_restriction_{i}.fa")
        sequnce = load_record(sequence_path)
        sequences[f'fragment {i}'] = str(sequnce.seq)
        desired_sequence += str(sequnce.seq)
    ...
    desired_sequence = dw.SequenceString(domesticated_sequence, metadata={"topology": 'circular'})
    ...
    assembly_report.write_full_report(f"Design")

    # now verify things are correct
    df = pd.read_csv(f'Design/sequences.csv', sep=';')
